...
/** Resolve USD prices for every mint referenced by options and baskets. */
export function usePriceMap(
  ...
  const mints = useMemo(() => {
    ...
    options.forEach((o) => o.account.underlyingMint?.toBase58?.() && set.add(o.account.underlyingMint.toBase58()));
    baskets.forEach((b) =>
...
export function basketNav(basket: any, prices: Record<string, AssetPrice>): number {
  return (basket.account.components ?? []).reduce((sum: number, c: any) => {
    const symbol = c.mint.toBase58();
    const p = prices[symbol]?.usd ?? 0;
    return sum + (Number(c.amountPerUnit.toString()) / 1e6) * p;
  }, 0);
}
